...
class AjaxController extends Controller
{
    ...
    */
    public function create_patient(PatientRequest $request)
    {
        ...
        $attr['government_id'] = $request->government_id;
        $attr['region_id'] = $request->region_id;
    ...
    */
    public function edit_patient(PatientRequest $request,$id)
    {
        ...
        $patient->update([
                    ...
                    'date_pms'=>$request['gender'] != 'male' ? $request['date_pms'] : null ,
                    'hours_fasting'=>$request['hours_fasting'],
                    'dob'=>$request['dob'],
                    ...
                    'gland' => $request->gland == 1 ? 1 : 0,
                    'tumors' => $request->tumors == 1 ? 1 : 0,
                    'iron' => $request->iron == 1 ? 1 : 0,
                    'cortisone' => $request->cortisone == 1 ? 1 : 0,
                    'antibiotic' => $request->antibiotic == 1 ? 1 : 0,
    ...
    */
    public function get_doctors(Request $request)
    {
        if(isset($request->term))
        {
            $doctors=User::where('name','like','%'.$request->term.'%')->whereHas('roles', function ($q) {
                $q->where('role_id', 6);
            })->take(20)->get();
        }
        else{
    ...
    */
    public function get_labs(Request $request)
    {
        if(isset($request->term))
        {
            ...
            $labs=User::where('type','lab')->where('name','like','%'.$request->term.'%')->orWhere('lab_code','like','%'.$request->term.'%')->take(20)->get();
        }
        else{
            $labs = User::where('type','lab')->take(20)->get();
        }
    ...
    */
    public function get_reps(Request $request)
    {
        if(isset($request->term))
        {
            $reps=User::where('user_type','representative')->where('name','like','%'.$request->term.'%')->take(20)->get();
        }
        else{
            $reps = User::where('user_type','representative')->take(20)->get();
        }
    ...
    */
    ...
    /**
    * select2 get_rays_categories
    *
    * @access public
    * @var  @Request $request
    */
    public function get_rays_categories(Request $request)
    {
        if(isset($request->term))
        {
            $raysCategories=Ray_category::where('name','like','%'.$request->term.'%')->take(20)->get();
        }
        else{
            $raysCategories=Ray_category::take(20)->get();
        }

        return response()->json($raysCategories);
    }
    ...
    * @var  @Request $request
    */
    public function get_invoice_rays(Request $request)
    {
        if(isset($request->term))
        {
            $rays=Ray::where('name','like','%'.$request->term.'%')
                        ->orWhere('id','like','%'.$request->term.'%')
                        ->take(50)
                        ->with('category')
                        ->get();
    ...
    */
    ...
    {
        $test=Test::with('category')->find($request['test_id']);
    ...
    * @var  @Request $request
    */
    public function get_invoice_ray(Request $request)
    {
        $ray=Ray::with('category')->find($request['ray_id']);
        $ray['current_price']=$ray->price;
    ...
    */
    ...
    {
        $culture=Culture::with('category')->find($request['culture_id']);
        if(!empty($request['contract_id']))
        {
            $contract=ContractPrice::where([
                ['contract_id',$request['contract_id']],
            ...
            $culture['current_price']=$culture['price'];
        }
    ...
    */
    ...
    {
        $package=Package::with('category','tests.test')->find($request['package_id']);
        if(!empty($request['contract_id']))
        {
            $contract=ContractPrice::where([
                ['contract_id',$request['contract_id']],
            ...
            $package['current_price']=$package['price'];
        }
    ...
    */
    ...
    public function get_user_info(Request $request){
        $user = User::with('roles','roles.role')->find($request->id);
