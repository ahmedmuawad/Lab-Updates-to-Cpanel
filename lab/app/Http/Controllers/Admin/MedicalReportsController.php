    ...
    public function print_report_2(Request $request, $id)
    {
        ...
        foreach ($categories as $category) {
            ...
            if ($request['rays']) {
                ...
                $rays = GroupRay::where('group_id', $group['id'])
                    ->whereIn('id', explode(',', $request['rays']))
                    ->get();
                 $category['rays'] = $rays;
            } else{
    ...
    {     
        ...
        if ($request->has('result')) {
            ...
            foreach ($request['result'] as $key => $result) {
                ...
                if(isset($resultes1) && $this->calcPersentage($resultes1->newResults[$i]->result,$group_test_result->result )  ){
